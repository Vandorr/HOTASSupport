...
begin
  ...
  begin
    ...
    watchList[p][Length(watchList[p])-1].min_hotkey := String(GT_View.DataController.Values[i,2])[1];

    watchList[p][Length(watchList[p])-1].max_value := GT_View.DataController.Values[i,3];
    watchList[p][Length(watchList[p])-1].max_hotkey := String(GT_View.DataController.Values[i,4])[1];

    watchList[p][Length(watchList[p])-1].steps_count := GT_View.DataController.Values[i,5];
...
var i: Integer;
begin
  userDir := GetSpecialFolderPath(CSIDL_LOCAL_APPDATA)+'\HOTASSupport';
...
//AViewInfo.GridView.DataController.Values[]
  if (inCalibrationMode) and (calibrationStep = 0) and ((AViewInfo.Item.Index = 1) or (AViewInfo.Item.Index = 1)) and (calibrationRow = AViewInfo.RecordViewInfo.Index) then
    ACanvas.Brush.Color := clWebBurlywood;
  if (inCalibrationMode) and (calibrationStep = 1) and ((AViewInfo.Item.Index = 3) or (AViewInfo.Item.Index = 3)) and (calibrationRow = AViewInfo.RecordViewInfo.Index) then
    ACanvas.Brush.Color := clWebBurlywood;
end;
...
begin
  ...
  ADataController.Values[ADataController.FocusedRecordIndex,2] := '';
  ADataController.Values[ADataController.FocusedRecordIndex,3] := '0';
  ADataController.Values[ADataController.FocusedRecordIndex,4] := '';
...
begin
  ...
  for i := 0 to c-1 do
    begin
      ...
      GT_View.DataController.Values[GT_View.DataController.FocusedRecordIndex,2] := Ini.ReadString( 'Axis', IntToStr(i)+'_MinHotkey', '');
      GT_View.DataController.Values[GT_View.DataController.FocusedRecordIndex,3] := Ini.ReadString( 'Axis', IntToStr(i)+'_MaxValue', '');
      GT_View.DataController.Values[GT_View.DataController.FocusedRecordIndex,4] := Ini.ReadString( 'Axis', IntToStr(i)+'_MaxHotkey', '');
      GT_View.DataController.Values[GT_View.DataController.FocusedRecordIndex,5] := Ini.ReadInteger( 'Axis', IntToStr(i)+'_StepsCount', 0);
...
begin
  ...
  begin
    ...
    begin
      ...
      Ini.WriteString( 'Axis', inttostr(i)+'_MinHotkey', GT_View.DataController.Values[i,2]);
      Ini.WriteString( 'Axis', inttostr(i)+'_MaxValue', GT_View.DataController.Values[i,3]);
      Ini.WriteString( 'Axis', inttostr(i)+'_MaxHotkey', GT_View.DataController.Values[i,4]);
      Ini.WriteString( 'Axis', inttostr(i)+'_StepsCount', GT_View.DataController.Values[i,5]);
    end;
...
var i, j, k, position, calc_position: integer;
    max_value,min_value,step_length, key_count, key_count_prev: integer;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (lastPosition[i]>min_value) and (lastPosition[i]<max_value) and (watchList[i][j].steps_count>0) then
            begin
              calc_position := position;
              if calc_position >= max_value then
                calc_position := max_value;
              if calc_position <= min_value then
                calc_position := min_value;

              step_length := Round((max_value-min_value)/watchList[i][j].steps_count);

              key_count_prev := Round(lastPosition[i]/step_length);
              key_count := Round(calc_position/step_length);
              if key_count_prev<key_count then
              begin
              ...
              else if key_count_prev>key_count then
              begin
